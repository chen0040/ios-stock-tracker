...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {		
	...
	[[NewsArticleManager sharedInstance] reloadData: responseString];
	[responseString release];
...
-(id)init
{
	self=[super init];
	if(!self) return nil;
	[self.tabBarItem initWithTitle:@"News" image:[UIImage imageNamed:@"tabbar_prices.png"] tag:0];
	return self;
}
...
{
	NewsArticleManager* nam=[NewsArticleManager sharedInstance];
	if(nam.pageIndex >= nam.pageCount-1)
	{
		return;
	}
	nam.pageIndex+=1;
...
{
	NewsArticleManager* nam=[NewsArticleManager sharedInstance];
	int code_count=[nam.code_entries count];
	NSMutableString* message=[NSMutableString stringWithString: @" codes: "];
	for(int i=0; i != code_count; ++i)
	{
		[message appendFormat:@"%@\n", [[nam.code_entries objectAtIndex:i] description]];
	}
...
-(void) addSegmentedControl: (CGRect) frame
{
	...
	NSArray *buttonNames = [NSArray arrayWithObjects:@"All", @"News1", @"News2", @"News3", @"News4", @"News5", nil];
...
{
	self.tvc=[[NewsTableViewController alloc] init];
...
-(void)loadView
{
	self.view=[[[NSBundle mainBundle] loadNibNamed:@"NewsViewController" owner:self options:nil] lastObject];
